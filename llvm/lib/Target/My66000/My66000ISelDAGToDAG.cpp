...
// Inspired by X86 shouldAvoidImmediateInstFormsForSize
bool My66000DAGToDAGISel::shouldAvoidImmediate(SDNode *N) const {
  uint32_t UseCount = 0;

  // Ignore 5-bit signed immediates as they generate
  // minimum size store instructions
  auto *C = dyn_cast<ConstantSDNode>(N);
  if (C && C->getSExtValue() >= -16 && C->getSExtValue() <= 15)
    return false;
  ...
  for (SDNode::use_iterator UI = N->use_begin(),
    ...
    // For now, only count stores of immediates as real uses.
    if (User->getOpcode() == ISD::STORE &&
            User->getOperand(1).getNode() == N) {
      UseCount++;
      continue;
    }
    // FIXME - should we should count others
  }
  return (UseCount > 1);
}
